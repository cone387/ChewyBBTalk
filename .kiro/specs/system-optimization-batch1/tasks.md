...
- [x] 3. 提取 PrivacyLockOverlay 组件
  - [x] 3.1 创建 `mobile/src/components/PrivacyLockOverlay.tsx`
  ...
  - [x] 3.2 在 HomeScreen 中替换防窥遮罩 JSX 为 PrivacyLockOverlay
    - 删除 HomeScreen 中防窥锁定遮罩的 JSX 代码
    - 替换为 `<PrivacyLockOverlay ... />` 组件调用
    - _需求：1.2, 1.6_

- [x] 4. 提取 SearchBar 和 TagTabs 组件
  - [x] 4.1 创建 `mobile/src/components/SearchBar.tsx`
  ...
  - [x] 4.2 创建 `mobile/src/components/TagTabs.tsx`
  ...
  - [x] 4.3 在 HomeScreen 中替换搜索栏和标签栏 JSX
    - 删除 HomeScreen 中搜索栏和标签栏的 JSX 代码
    - 替换为 `<SearchBar ... />` 和 `<TagTabs ... />` 组件调用
    - 额外提取 `useTagSwipe` Hook（标签滑动切换逻辑）和 `useBBTalkActions` Hook（操作逻辑）
    - HomeScreen 总行数 286 行，低于 300 行目标 ✅
    - _需求：1.3, 1.4, 1.6, 1.7_

- [x] 5. 检查点 — HomeScreen 拆分验证
  - HomeScreen.tsx 行数 286 行 < 300 行 ✅
  - 所有子组件（BBTalkCard、PrivacyLockOverlay、SearchBar、TagTabs）和 Hook（usePrivacyMode、useTagSwipe、useBBTalkActions）可正常导入 ✅
  - TypeScript 编译无错误 ✅

- [x] 6. Web 端 Redux 扩展与删除撤销功能
  - [x] 6.1 在 Web 端 bbtalkSlice 中添加 `optimisticDelete` 和 `undoDelete` reducers
...
- [x] 7. Web 端 UndoToast 和 SkeletonCard 组件
  - [x] 7.1 创建 `frontend/src/components/UndoToast.tsx`
  ...
  - [x] 7.2 创建 `frontend/src/components/SkeletonCard.tsx`
  ...
  - [x] 7.3 在 BBTalkPage 中集成乐观删除、UndoToast 和 SkeletonCard
...
- [x] 8. 检查点 — Web 端功能验证
  - Web 端 TypeScript 编译无错误 ✅
  - 乐观删除 + UndoToast 撤销流程已集成 ✅
  - SkeletonCard 在首次加载时显示、数据加载后消失 ✅

- [x] 9. Mobile 端统一错误处理模块
  - [x] 9.1 创建 `mobile/src/utils/errorHandler.ts`
  ...
  - [x] 9.3 替换现有静默 catch 块
    - 在 HomeScreen 中将 `catch {}` 替换为 `catch (e) { logError(e, 'context') }`
    - 在 usePrivacyMode.ts 中将生物识别检测的 `catch {}` 替换为 `logError`
    - 涉及文件：HomeScreen.tsx、usePrivacyMode.ts、useBBTalkActions.ts
    - _需求：3.3, 3.4, 3.5, 3.6_

- [x] 10. Mobile 客户端版本检测更新
  - [x] 10.1 安装 `expo-updates` 依赖
    - 在 `mobile/` 目录下安装 `expo-updates` 包
    - 在 `app.json` 中添加 `updates` 配置和 `runtimeVersion` 策略
    - 在 `plugins` 中添加 `expo-updates`
    - _需求：4.2_

  - [x] 10.2 创建 `mobile/src/utils/versionChecker.ts`
...
- [x] 11. 列表性能优化（React.memo + useCallback）
  ...
  - [x] 11.3 在 HomeScreen 中使用 useCallback 包裹所有回调
    - 使用 `useCallback` 包裹 `renderItem`（FlatList 的渲染函数）
    - 所有传递给 BBTalkCard 的回调已在 `useBBTalkActions` Hook 中用 `useCallback` 包裹
    - `showLocation`、`onNavigateCompose` 等回调在 HomeScreen 中用 `useCallback` 包裹
    - 确保 `useCallback` 的依赖数组正确，避免闭包陷阱
    - _需求：5.1, 5.4_

- [x] 12. 最终检查点 — 全部功能验证
  - Mobile TypeScript 编译无错误 ✅
  - Web TypeScript 编译无错误 ✅
  - HomeScreen 行数 286 行 < 300 行（需求 1.7）✅
  - Web 端删除撤销和骨架屏功能已集成（需求 2）✅
  - ErrorHandler 正确分类错误并显示中文提示（需求 3）✅
  - 版本检测已集成到 App.tsx，不影响 App 正常启动（需求 4）✅
  - BBTalkCard 使用 arePropsEqual + useCallback 优化渲染性能（需求 5）✅
  - 属性测试（标记 * 的可选任务）已跳过以加速交付

## 备注

- 标记 `*` 的任务为可选任务，已跳过以加速 MVP 交付
- 每个任务引用了具体的需求编号，确保可追溯性
- 检查点任务确保增量验证，及时发现问题
- 需求 1（HomeScreen 拆分）和需求 5（性能优化）存在联动：BBTalkCard 的 React.memo 在任务 2 中创建骨架，在任务 11 中完善比较函数
- 额外提取了 `useTagSwipe` 和 `useBBTalkActions` 两个 Hook，进一步降低 HomeScreen 复杂度
