...
class LocalStorage(FileSystemStorage):
    ...
    def fetch(self, url):
        ...
        return File(file, name=urlparse(url).path)
...
class QiniuStorage(Storage):
    ...
    def _normalize_name(self, name):
        ...
        return urlparse(name).path.lstrip('/')

    def fetch(self, url, name=None):
        name = name or urlparse(url).path[1:]
        ret, info = self.bucket_manager.fetch(url, self.bucket_name, name)
        if ret is None:
            raise QiniuError(info)
        return QiniuFile(name, self, 'rb')
...
class QiniuFile(File):
    def __init__(self, name, storage, mode):
        ...
        self._is_read = False
        super(QiniuFile, self).__init__(file=six.BytesIO(), name=name)
